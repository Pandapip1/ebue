...
"""Check pointer provenance proof diagnostics."""
...
FIXTURES = ROOT / "tools/lint-pointer-provenance-fixtures"
...
def parse(path: pathlib.Path) -> list[Finding]:
    ...
    if "PLEASE submit a bug report" in text or "clang frontend command failed" in text:
        raise SystemExit(f"lint-pointer-provenance: analyzer crashed; see {path}")
    result = []
...
def check_fixtures(path: pathlib.Path) -> None:
    expected = {(source.relative_to(ROOT).as_posix(), number)
                ...
                if "pointer-provenance-expect" in line}
    actual = {(finding.path, finding.line) for finding in parse(path)}
    if expected != actual:
        print(f"lint-pointer-provenance: fixture mismatch\n  expected: {sorted(expected)}\n"
...
def main() -> int:
    ...
    if findings:
        print(f"lint-pointer-provenance: {len(findings)} unproved operation(s)")
        return 1
    print("lint-pointer-provenance: no findings (fixtures passed)")
    return 0
