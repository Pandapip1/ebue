...
"""Check that results from known fallible APIs are not discarded."""
...
FIXTURES = ROOT / "tools/lint-fallible-result-fixtures"
...
def parse(path: pathlib.Path) -> list[Finding]:
    ...
    if "PLEASE submit a bug report" in text or "clang frontend command failed" in text:
        raise SystemExit(f"lint-fallible-result: analyzer crashed; see {path}")
    result = []
...
def check_fixtures(path: pathlib.Path) -> None:
    expected = {(source.relative_to(ROOT).as_posix(), number)
                ...
                if "fallible-result-expect" in line}
    actual = {(finding.path, finding.line) for finding in parse(path)}
    if expected != actual:
        print(f"lint-fallible-result: fixture mismatch\n  expected: {sorted(expected)}\n"
...
def main() -> int:
    ...
    if findings:
        print(f"lint-fallible-result: {len(findings)} discarded fallible result(s)")
        return 1
    print("lint-fallible-result: no findings (fixtures passed)")
    return 0
