...
not_native()
{
	...
	sh-main)
		echo "spawns obj/sh/sh.exe, a PE program built by \$(CC) from sh/main.c -- this build compiles src/*.c natively and produces no such binary at all, so the test would find nothing to exercise (it exits 77 rather than passing vacuously). What it covers is the shell *utility's* argument handling, exit status and diagnostics, which needs a real process; the engine those diagnostics come from is src/sh/*.c, which this build does compile and test/sh-engine.c does run here. Covered by 'make check' under Wine (and real Windows CI)" ;;
	*)  echo "" ;;
	esac
}
