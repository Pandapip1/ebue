...
# (third_party/libc-test, vendored at the pinned SHA recorded in
# third_party/libc-test/VENDORING.md) against this library, and
...
#     test/libc-test-expected.txt.  A vendored test with no line is a
#     hard error, not a default-to-ignored.
...
# THE MATH CORPUS
#
# Not vendored and not run by default; `tools/libc-test.sh math` says so
# and exits non-zero unless pointed at a checkout.  See the `math` case
# at the bottom for the two reasons (licence, and 82 untriaged failures).
#
# Usage:
...
if [ "${1:-}" = "math" ]; then
	# Deliberately not vendored, for two independent reasons:
	#
	#  1. Licence.  src/math/ucb/ and src/math/crlibm/ are BSD- and
	#     GPL-licensed third-party vectors (libc-test's own COPYRIGHT
	#     says so); the rest of the corpus we vendor is uniformly MIT.
	#     9.6 MB of mixed-licence test vectors is a REUSE liability for
	#     a corpus nobody has triaged yet.
	#  2. It would nearly triple the size of this repository (5.6 MB
	#     today) for a stage that external-suites.md explicitly says
	#     should stay on demand until its 82 failures are triaged.
	#
	# What it must NOT do is quietly succeed.  No checkout, no run, no
	# zero exit.
	if [ -z "${LIBC_TEST_MATH:-}" ]; then
		echo "libc-test: the math corpus is not vendored in this tree." >&2
		echo "libc-test: 82 of its 174 linkable tests fail here and none of" >&2
		echo "libc-test: them has been triaged (test/external-suites.md)." >&2
		echo "libc-test: To run it anyway, clone git://repo.or.cz/libc-test.git" >&2
		echo "libc-test: at the SHA in third_party/libc-test/VENDORING.md and" >&2
		echo "libc-test: re-run with LIBC_TEST_MATH=/path/to/that/checkout." >&2
...
if [ -z "$corpus" ]; then
	echo "libc-test: the vendored corpus is empty ($SUITE)." >&2
	exit 1
fi
...
if [ -n "$absent" ]; then
	echo
	echo "libc-test: FAILED -- test(s) in the vendored corpus with no line in"
