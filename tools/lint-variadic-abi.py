...
"""Check Clang's printf/scanf variadic ABI diagnostics."""
...
FIXTURES = ROOT / "tools/lint-variadic-abi-fixtures"
...
class Finding:
    ...
    def key(self) -> tuple[str, str, str]:
        return self.path, self.reason, self.warning
...
def parse(path: pathlib.Path) -> list[Finding]:
    ...
    if "PLEASE submit a bug report" in text or "clang frontend command failed" in text:
        raise SystemExit(f"lint-variadic-abi: compiler crashed; see {path}")
    result = []
    for line in text.splitlines():
        match = DIAGNOSTIC.match(line)
        if match:
            result.append(Finding(relative(match.group(1)), match.group(4),
                                  match.group(5), int(match.group(2))))
    return result
...
def check_fixtures(path: pathlib.Path) -> None:
    expected = {(source.relative_to(ROOT).as_posix(), number)
                ...
                if "variadic-abi-expect" in line}
    actual = {(finding.path, finding.line) for finding in parse(path)}
    if expected != actual:
        print(f"lint-variadic-abi: fixture mismatch\n  expected: {sorted(expected)}\n"
...
def main() -> int:
    ...
    for finding in sorted(findings.values()):
        print(f"{finding.path}:{finding.line}: {finding.reason} [{finding.warning}]")
    if findings:
        print(f"lint-variadic-abi: {len(findings)} unproved variadic call(s)")
        return 1
    print("lint-variadic-abi: no findings (fixtures passed)")
    return 0
