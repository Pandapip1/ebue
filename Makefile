...
# test/util-findls.c is the same idea, one tier up: the first Tier-4
# "bigger engines" batch -- find(1p)/xargs(1p)/expr(1p)/ls(1p) -- each
# spawned as obj/bin/<name>.exe and exercised as a shell built-in via
# obj/sh/sh.exe -c, so all four .exes plus the shell need to exist first.
