...
# test/util-diffcmp.c is the same idea, one tier up: the "bigger
# engines" tier -- diff(1p)/cmp(1p), each spawned as obj/bin/<name>.exe
# and exercised as a shell built-in via obj/sh/sh.exe -c, so both .exes
# plus the shell need to exist first.
