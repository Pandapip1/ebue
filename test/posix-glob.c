...
#if NTLIBC_TEST(BUG, posix_glob_wordexp_append_preserved_on_error) /* BUG: XCU 2.6 Word Expansions, step 2 -- "Field splitting ...
...
#if NTLIBC_TEST(BUG, posix_glob_wordexp_honours_ifs) /* BUG: XCU 2.6.5 Field Splitting -- "the shell shall treat each
...
#if NTLIBC_TEST(BUG, posix_glob_wordexp_empty_field_deleted) /* BUG: XCU 2.6 Word Expansions -- "If the complete expansion
...
#if NTLIBC_TEST(BUG, posix_glob_wordexp_append_preserved_on_error) /* BUG: wordexp.html DESCRIPTION -- "if words contains an
