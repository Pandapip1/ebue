 ...
 * and run 2 beginning "root/[x]".  ntlibc returns the same, entry for
 * entry.
 *
...
#if NTLIBC_TEST(PASS, posix_glob_wordexp_line_continuation) /* wordexp() performs <backslash><newline> line
...
#if NTLIBC_TEST(PASS, posix_glob_wordexp_append_preserved_on_error) /* XCU 2.6 Word Expansions, step 2 -- "Field splitting ...
...
#if NTLIBC_TEST(PASS, posix_glob_wordexp_honours_ifs) /* XCU 2.6.5 Field Splitting -- "the shell shall treat each
...
#if NTLIBC_TEST(PASS, posix_glob_wordexp_empty_field_deleted) /* XCU 2.6 Word Expansions -- "If the complete expansion
...
#if NTLIBC_TEST(PASS, posix_glob_wordexp_append_preserved_on_error) /* wordexp.html DESCRIPTION -- "if words contains an
...
#if NTLIBC_TEST(NA, posix_glob_nftw_phys_and_symlinks) /* N/A: nftw.html FTW_PHYS vs symlink-following -- fixture needs a working symlink(), see above */
