...
#if 0 /* UNIMPL: psiginfo.html DESCRIPTION: "The psiginfo() function
 * shall write a message to the standard error stream ... If the
 * argument message is not a null pointer, ... message ... followed by
 * a colon character and a <space> ... followed by [text for
 * pinfo->si_signo], as if generated by strsignal() ... followed by a
 * <newline>. If message is a null pointer, only [that text] shall be
 * written." RETURN VALUE: "no return value." grepping include/ and
 * src/ finds no `psiginfo` symbol or prototype anywhere -- unlike
 * psignal.html's other half (psignal(), see test_strsignal() below,
 * likewise absent), this is self-contained: it needs only strsignal()
 * (already implemented, src/signal/) plus stdio, not any missing
 * asynchronous-delivery subsystem. Implementable without touching
 * signal.c at all. */
