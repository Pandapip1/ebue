...
### N7: never judge a parallel worker by `wait`

**Catches: a vacuous-success generator that sits in the default shape of
the obvious solution. Four drivers in this tree are exposed to it.**

Three of this tree's runners were parallelised in one day --
`tools/lint.sh`'s stage dispatch, `tools/linkcheck.sh`'s per-symbol
loop, `tools/asan-build.sh`'s compiles and test links -- and
`tools/gate.sh` was already. All four use the same shape: launch N
background subshells, `wait`, then report. The obvious way to decide
whether the run succeeded is to look at what `wait` returned.

It is worthless. Measured, in `/bin/sh`, `/bin/bash` and `/bin/dash`
alike:

```
$ sh -c 'set -eu; ( exit 3 ) & wait; echo "wait rc=$?; continued"'
wait rc=0; continued
```

`wait` **with no operands returns zero**, whatever its children did, and
`set -e` does not save you because there is nothing non-zero to trip on.
The same shell propagates correctly the moment an operand is given:

```
$ sh -c 'set -eu; ( exit 3 ) & p=$!; wait "$p"; echo continued'   # exits 3, prints nothing
```

So a parallel driver that judges its workers with bare `wait` **reports
success no matter what they did** -- a worker that died on SIGPIPE, one
that was OOM-killed, one whose loop never ran because its shard file was
empty, all indistinguishable from a clean run. That is class A from the
first document, generated by the idiom everyone reaches for first, and
it is worse than the instances that document catalogues: those were
checks that *could* not tell they had done nothing, whereas this one
actively answers "fine" when asked.

**The measure is a convention, and all four drivers already satisfy it
-- by luck, not by knowledge.** Every worker writes a result file, on
success as well as failure, and a serial pass counts those files against
the work list. A worker that never reported is a named failure, not a
smaller number:

| Driver | The count |
|--------|-----------|
| `tools/gate.sh:312` | `$reported` against `$expected`, naming the stages that never wrote a `.rc` |
| `tools/lint.sh` | same, per lint stage |
| `tools/linkcheck.sh` | `$nreported` against `$checked`, plus a per-symbol `NO RESULT` line |
| `tools/asan-build.sh` | one `.rc` per compile and per test link, counted in the serial merge |

Each of those exists because the floors work asked "how does this report
success having done nothing", not because anyone knew `wait` was
lying. The convention is worth writing down precisely so the next
parallel driver does not have to rediscover it: **a worker's exit status
is not evidence; the artefact it produced is.**

**Cost.** Zero. It is a rule about how to write the next one, and a
one-line audit of the four that exist (`grep -n '^\s*wait\s*$'
tools/*.sh` -- five hits, four files).

**Honest limit.** This says nothing about correctness, exactly as the
first document's M1 says nothing about correctness. It guarantees that a
parallel stage which stops doing its work says so. Given that
parallelising the gate's stages is the only remaining source of
wall-clock headroom -- `asan` is the critical path and was entirely
serial -- the number of drivers this applies to is going up, not down.
...
| -- | **N7** never judge a parallel worker by `wait` | 0 s, a convention | nothing yet; four drivers in this tree are one edit away from it | anything about correctness -- it only guarantees a parallel stage says so when it stops working |
...
One cost note, inherited and **now wrong, which is itself the
interesting part**. `tools/gate.sh` runs stages concurrently, so the
question for any addition is a question about the critical path, not
about the sum. Both documents phrased that as "is it slower than
`linkcheck`" -- and that phrasing was calibrated against a gate in which
`make asan` exited immediately having linked nothing, which is the first
document's defect 1. Fixing defect 1 made `asan` run its 282 compiles
and 54 tests for real and moved the critical path there, where it has
stayed. Measured on an idle machine after the fix: `asan` 42 s,
`linkcheck` 9 s, every other stage below that.

So the vacuous stage did not merely fail to catch bugs. It silently
recalibrated every performance judgement made afterwards, including the
ranking advice in both of these documents. `linkcheck` has since been
parallelised anyway (9.9 s to 2.5 s, and its report is byte-identical at
any job count), which buys zero gate wall clock and was worth doing for
`make linkcheck` by hand. `asan` has been parallelised for its compiles
and its test links, which is the two thirds of it that is safe to run
concurrently; its test *runs* stay serial for the reason recorded above
`tools/asan-build.sh`'s link phase. Every measure ranked above is still
under a second, and the correct question is now "is it slower than
`asan`".
