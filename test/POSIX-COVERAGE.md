...
| fchmodat | `[EPERM]`, `[EACCES]`, `[EROFS]`, `[ELOOP]` (both forms), `[ENAMETOOLONG]` | N/A / UNIMPL (fenced) — no ownership model to violate, and no symbolic link to build a loop with — a Wine version gap rather than a privilege one, see `renameat`'s row above; `[ENAMETOOLONG]` is the same whole-path-vs-component mismatch as `renameat`'s | `test_fchmodat_eperm`, `test_fchmodat_eloop`, `test_fchmodat_enametoolong` |
...
| `limits.h` | `limits.h.html` "Minimum Values" — "The `<limits.h>` header shall define the following symbolic constants with the values shown", for the three entries carrying **no** option-group marker | **ABSENT** | **UNIMPL (fenced)** — `_POSIX_THREAD_DESTRUCTOR_ITERATIONS`, `_POSIX_THREAD_KEYS_MAX`, `_POSIX_THREAD_THREADS_MAX`. These state what the *standard* guarantees, not what ntlibc provides, so — unlike `<unistd.h>`'s `_POSIX_THREADS`, where a definition would be a false capability claim — the acceptance criterion really is three `#define`s. Observed: fails to **compile**, `'_POSIX_THREAD_DESTRUCTOR_ITERATIONS' undeclared` | `test/posix-limits.c` fence `test_limits_minimum_values_unmarked` |
| `limits.h` | `limits.h.html` "Minimum Values", same sentence, for its three `[XSI]` entries | **ABSENT** | **UNIMPL (fenced)** — `_XOPEN_IOV_MAX` (16), `_XOPEN_NAME_MAX` (255), `_XOPEN_PATH_MAX` (1024). Fenced apart from the unmarked three because the triage differs: an absent XSI constant is a missing option group a conformant implementation may lack. Fenced anyway because ntlibc does not decline XSI — it compiles `-D_XOPEN_SOURCE=700`, implements `readv()`/`writev()` and publishes `IOV_MAX`. **Evidence the gap was seen and not recorded** — the whole point of group U: `test_limits_pathname()` asserts `IOV_MAX >= 16` under the comment "XSI floor is `{_XOPEN_IOV_MAX}` == 16 (not defined as a macro by ntlibc, so the spec's literal is used directly here)". A previous audit hit the absence, worked around it with a literal, and left no fence, no ledger row and no header note — from outside, indistinguishable from "checked, fine". Un-fencing should turn that literal back into `_XOPEN_IOV_MAX`. Observed: fails to **compile**, `'_XOPEN_IOV_MAX' undeclared` | `test/posix-limits.c` fence `test_limits_minimum_values_xsi` |
| `limits.h` | `limits.h.html` "Runtime Increasable Values" — "The magnitude limitations in the following list shall be fixed by specific implementations. An application should assume that the value of the symbolic constant defined by `<limits.h>` ... is the minimum that pertains" | **ABSENT** | **UNIMPL (fenced)** — 9 of 10 absent: `BC_BASE_MAX`, `BC_DIM_MAX`, `BC_SCALE_MAX`, `BC_STRING_MAX`, `CHARCLASS_NAME_MAX`, `COLL_WEIGHTS_MAX`, `EXPR_NEST_MAX`, `LINE_MAX`, `RE_DUP_MAX`; only `NGROUPS_MAX` present. This section grants **no** permission to omit — the value may be *increased* at run time and reported via `sysconf()`, but the header still publishes the compile-time minimum. **Highest consumer impact in the `<limits.h>` group**: `LINE_MAX` sizes a line-oriented utility's input buffer (sed, awk, the coreutils text tools — the packages this libc exists to build), `RE_DUP_MAX` bounds a BRE/ERE interval expression, `COLL_WEIGHTS_MAX`/`CHARCLASS_NAME_MAX` are read by locale-aware regex and collation. All the `_POSIX2_*` floors are present, so each has something to be checked against the moment it exists. Observed: fails to **compile**, `'BC_BASE_MAX' undeclared` | `test/posix-limits.c` fence `test_limits_runtime_increasable` |
| `limits.h` | `limits.h.html` "Other Invariant Values" — "The `<limits.h>` header shall define the following symbolic constants:" | **ABSENT** | **UNIMPL (fenced)** — all six absent: `NL_ARGMAX` (≥9), `NL_LANGMAX` `[XSI]` (≥14), `NL_MSGMAX` (≥32767), `NL_SETMAX` (≥255), `NL_TEXTMAX` (≥`_POSIX2_LINE_MAX`), `NZERO` `[XSI]` (≥20). `NL_ARGMAX` bounds *n* in a `"%n$"` conversion specification and ntlibc's `printf` implements no positional arguments at all — consistent, but consistency is not conformance, and a consumer sizing its own argument table gets a compile error rather than a small number. The fence claims nothing about adding `%n$`. Observed: fails to **compile**, `'NL_ARGMAX' undeclared` | `test/posix-limits.c` fence `test_limits_other_invariant` |
