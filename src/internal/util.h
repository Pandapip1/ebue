...
/* Tier 2: text-formatting/file-splitting utilities (XCU printf(1p),
 * od(1p), pr(1p), tabs(1p), split(1p), csplit(1p)).  Alphabetical, same
 * as the tier above.  None is __pure__: printf/od/pr/tabs write to
 * stdout unconditionally as their whole purpose, and split/csplit do
 * real filesystem I/O creating the piece files.  Each gets nonnull(2)
 * for the same reason as the tier above: a real argv from a real caller
 * is never NULL, and each function's own usage-error path formats
 * argv[0] into a diagnostic before any argc check could matter. */
