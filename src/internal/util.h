...
/* Tier 4: the first "bigger engines" batch (find(1p), xargs(1p),
 * expr(1p), ls(1p)) -- real expression grammars and a real recursive
 * traversal, not just a straight-line pass over stdin/argv the way most
 * of Tiers 0-3 are.  Each has its own real, deliberate scope narrowing
 * documented at length in its own src/util/<name>.c header comment
 * (find: -H/-L, XSI-only -perm symbolic-mode grammar, -xdev/-nouser/
 * -nogroup; xargs: -0/-d, -E's default-enabled state; ls: -H/-L, the
 * two XSI-shaded -g/-o long-format variants) rather than being silently
