...
/* Tier 2: text-formatting utilities (XCU cut(1p), paste(1p), tr(1p),
 * expand(1p), unexpand(1p), fold(1p)).  Alphabetical, like the block
 * above.  None is __pure__: all six read standard input or a file
 * operand and write to standard output, so a repeated call is not
 * guaranteed to see the same bytes twice (a pipe, a file another
 * process is still writing, etc.) even though none of them ever writes
 * anything back to the filesystem the way the block above's mkdir/
 * rmdir/mkfifo/ln/chmod/touch do. */
