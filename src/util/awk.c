 ...
 *
 * A FATAL RUNTIME CONDITION -- division by zero, a call to an undefined
 * function, a scalar used as an array or the reverse, runaway
 * user-function recursion -- is a DIFFERENT case from the allocation
 * failure above and does not exit() the process, unlike the parser and
 * interpreter's OOM path: this file's own __util_awk_main() can run
 * in-process as a shell built-in (bi_awk(), src/sh/builtin.c, no
 * fork()), and every one of those conditions is something an ordinary
 * awk PROGRAM can reach on its very first record, not a resource
 * exhaustion a fuzzer or an attacker would need to induce deliberately.
 * The setjmp() immediately below is where awk_run.c's fatal() (see that
 * file's own header, and awk_priv.h's struct awk_interp comment on
 * `fatal_jmp`) unwinds to instead of calling exit(2): __util_awk_main()
 * returns ip.exit_status exactly as it would from an ordinary
 * awk_interp_run() completion, so a bad one-liner run as a builtin ends
 * the `awk` command, not the shell running it.
 *
...
int __util_awk_main(int argc, char **argv)
{
	...
	/* Catches a longjmp() from awk_run.c's fatal() -- see this file's
	 * own header ("A FATAL RUNTIME CONDITION...") and awk_priv.h's
	 * struct awk_interp comment on `fatal_jmp`.  This has to wrap every
	 * awk_interp_*() call below, not just awk_interp_run(): fatal()'s
	 * one caller outside awk_interp_run() is
	 * awk_interp_setup_argv()/awk_interp_setup_environ()'s own
	 * promote_to_array(), reachable if a -v assignment already bound
	 * ARGV or ENVIRON to a scalar (e.g. `awk -v ARGV=1 'BEGIN{}'`)
	 * before either is called.  ip.exit_status is set by fatal() itself
	 * before the jump, so the non-zero setjmp() return here needs
	 * nothing further from the condition that triggered it. */
	if (setjmp(ip.fatal_jmp)) {
