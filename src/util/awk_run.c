 ...
 * awk_parse.c's own header explains for the parser -- oom() below still
 * calls exit(2) directly, matching src/util/find.c's own out_of_memory()
 * (the one other Tier-4 utility that also makes this choice), on the
 * reasoning that a real allocation failure is not something a fuzzed or
 * adversarial awk PROGRAM can induce by itself, unlike every one of the
 * fatal() conditions just below.
 *
 * A FATAL RUNTIME CONDITION IS NOT THE SAME THING, and does not use
 * exit(): division by zero, a call to an undefined function, using a
 * scalar as an array (or the reverse), and runaway user-function
 * recursion are all things an ordinary -- not even malformed, just
 * unlucky -- awk PROGRAM can trigger on its very first record, and
 * __util_awk_main() (src/util/awk.c) can run in-process as a shell
 * builtin (src/sh/builtin.c's bi_awk(), no fork(): see
 * src/internal/util.h's header for why every __util_<name>_main() must
 * return a real exit status rather than ever exit() the process, and
 * src/util/dd.c's for the same discipline argued for a signal handler).
 * exit()ing here would kill the whole host shell over one bad awk
 * one-liner, not just this command.  fatal() instead sets
 * ip->exit_status and longjmp()s to the one setjmp() __util_awk_main()
 * establishes around the whole init/setup/run sequence -- ip->unwind
 * (the next/exit mechanism described above) cannot be reused for this,
 * because it relies on every intermediate caller cooperatively checking
 * it after each sub-call, and a fatal condition can be discovered
 * arbitrarily deep inside eval()'s own recursion (or even before
 * awk_interp_run() is ever reached: awk_interp_setup_argv()'s
 * promote_to_array(ARGV) can hit "can't use a scalar value as an array"
 * if a `-v ARGV=1` already bound ARGV to a scalar), with no cooperative
 * chain to thread a return value through cheaply.  See awk_priv.h's
 * struct awk_interp comment on the `fatal_jmp` field for where the
 * setjmp() side lives.
 */
#include <stdlib.h>
#include <string.h>
...
/* See this file's own header ("A FATAL RUNTIME CONDITION...") for why
 * this is longjmp(), never exit(). Streams opened so far (print
 * redirections, coprocesses) are flushed here rather than left to the
 * catch site, since the catch site (__util_awk_main(), a different
 * translation unit) has no access to this file's static stream table. */
static void fatal(struct awk_interp *ip, const char *msg)
{
	__util_diagf("awk: %s\n", msg);
	ip->exit_status = 2;
	flush_all_streams(ip);
	longjmp(ip->fatal_jmp, 1);
}
...
static char *num_to_str_fmt(double n, const char *fmt)
{
	...
	if (isfinite(n) && n > -1e18 && n < 1e18) {
		...
		is_int = ((double)ll == n);
		if (is_int) { snprintf(buf, sizeof buf, "%lld", ll); return xstrdup(buf); }
	}
	if (snprintf(buf, sizeof buf, fmt, n) < 0) buf[0] = 0;
	return xstrdup(buf);
}
...
static void v_str_init(struct awk_value *v, char *s, unsigned char kind) { v->num = 0; v->str = s; v->kind = kind; v->numcached = 0; v->strcached = 1; }
...
/* Every awk value that reaches an integer context here -- a field
 * index ($N), NF=, printf's %d/%o/%u/%x/%X/%c and its own "*" (star)
 * width/precision, exit's status code -- used to go straight from
 * v_num()'s double into `(long)`/`(int)`/`(long long)` with a bare C
 * cast. That is undefined behaviour, not just an inconvenient result,
 * the moment the double does not fit the target type (C11 6.3.1.4p1)
 * -- and every one of those doubles comes from strtod() on program
 * text or field/input data the user (or here, the fuzzer) controls
 * directly, so `$1e300` or `NF=1e300` reaches it on the first record.
 * fuzz/fuzz_awk.c's campaign found this at the $N call site (as
 * `$111111111111111111111 = ...`) within its first fuzzed minute, and
 * it is not merely academic there: eval_lvalue_read()/assign_lvalue()
 * fed the resulting (implementation-garbage, on real hardware often
 * LONG_MIN) `long` straight into set_field()'s array-growth and
 * indexing math, which is a real out-of-bounds write, not a
 * theoretical one -- see set_field()'s and set_nf()'s own comments on
 * the MAX_FIELD guard this same finding motivated.
 *
 * Clamping the double into long long's range *before* the cast, rather
 * than bounds-checking the integer after, is what actually removes the
 * UB: the cast below never sees a value its target cannot represent.
 * A caller that wants `long` or `int` narrows this function's result
 * with an ordinary cast afterward, which is only implementation-defined
 * (C11 6.3.1.3p3), not undefined -- narrowing a value that already fits
 * long long into a smaller signed type has a defined (if possibly
 * surprising) outcome, unlike casting an out-of-range double directly.
 * NaN clamps to 0 rather than falling into the range comparisons below
 * -- every comparison against NaN is false, so without the explicit
 * check it would fall through to the plain `(long long)d` cast, itself
 * UB for a NaN operand. 0 is an arbitrary but safe choice among the
 * results XCU's own "unspecified" allowance covers for a value with no
 * defined ordering. */
static long long v_num_ll(double d)
{
	if (d != d) return 0;                            /* NaN */
...
static const char *convfmt_str(struct awk_interp *ip)
{
	...
	if ((c->kind == VK_STR || c->kind == VK_STRNUM) && c->strcached) return c->str;
	return "%.6g";
}
...
static struct awk_value cell_to_value(struct awk_interp *ip, struct awk_cell *c)
{
	...
	v.str = c->strcached ? xstrdup(cell_str(ip, c)) : NULL;
...
static void promote_to_array(struct awk_interp *ip, struct awk_cell *c)
{
	if (c->is_array) return;
	if (c->kind != VK_UNINIT) fatal(ip, "can't use a scalar value as an array");
	c->is_array = 1;
...
/* An upper bound on a field index/NF this file will ever trust for
 * array growth or indexing -- both set_field() (`$N = ...`, N from
 * program text or input) and set_nf() (`NF = ...`) used to pass an
 * arbitrary, program-controlled `long` straight to fields_reserve(),
 * whose own parameter is `int`: fields_reserve(ip, (int)idx) silently
 * truncates any idx past INT_MAX/INT_MIN (C11 6.3.1.3p3 -- implemen-
 * tation-defined, not undefined, but still the wrong size for what
 * comes next), and the indexing that follows it in both functions
 * (ip->flds[idx - 1] in set_field(); the `for` loop bound by the
 * untruncated `newnf` in set_nf()) then used the *original*, huge
 * `long` -- a real out-of-bounds write on an array sized for the
 * truncated value, not a merely theoretical one: fuzz/fuzz_awk.c's
 * campaign reached `$111111111111111111111 = ...` within its first
 * fuzzed minute (see v_num_ll()'s own header for the double-to-integer
 * half of this same finding). No real awk record or NF assignment
 * needs anywhere near this many fields; one bound shared by both
 * functions is simpler than reasoning about int-truncation separately
 * in each. */
...
static void set_field(struct awk_interp *ip, long idx, struct awk_value *val)
{
	...
	if (idx > MAX_FIELD) return; /* same treatment, same reason -- see MAX_FIELD's own comment */
	if (idx > ip->nf) {
...
static void set_nf(struct awk_interp *ip, long newnf)
{
	int i;
	if (newnf < 0) newnf = 0;
	if (newnf > MAX_FIELD) newnf = MAX_FIELD; /* see MAX_FIELD's own comment */
	if (newnf < ip->nf) {
...
static FILE *get_output_stream(struct awk_interp *ip, const char *target, enum awk_redir redir)
{
	...
	if (!st) oom();
	st->is_input = 0;
	if (redir == RD_PIPE) { st->is_pipe = 1; st->f = popen(target, "w"); }
	else { st->is_pipe = 0; st->f = fopen(target, redir == RD_APPEND ? "a" : "w"); }
	if (!st->f) { free(st); *slot = NULL; awk_htab_del(&ip->streams, target, NULL); return NULL; }
	*slot = st;
	return st->f;
}

static FILE *get_input_stream(struct awk_interp *ip, const char *target, int is_pipe)
{
	void **slot = awk_htab_getp(&ip->streams, target);
	struct awk_stream *st;
	if (!slot) oom();
...
static int advance_to_next_argv_file(struct awk_interp *ip)
{
	...
	for (;;) {
		long argc = (long)v_num_ll(cell_num(argc_cell));
		char key[32];
		void *slot;
		const char *s;
...
static char *awk_format(struct awk_interp *ip, const char *fmt, struct awk_value *args, int nargs)
{
	...
	while (*p) {
		...
		if (p[1] == '%') { buf_append(&out, &len, &cap, "%", 1); p += 2; continue; }
		{
			...
			flags[nflags] = 0;
			if (*p == '*') { width = (ai < nargs) ? (int)v_num_ll(v_num(&args[ai++])) : 0; p++; }
			else while (isdigit((unsigned char)*p)) { width = (width < 0 ? 0 : width) * 10 + (*p++ - '0'); }
			if (*p == '.') {
				p++;
				if (*p == '*') { prec = (ai < nargs) ? (int)v_num_ll(v_num(&args[ai++])) : 0; p++; }
			 ...
			 * then once for real into an exactly-sized allocation. */
			{
				...
				switch (conv) {
				case 'd':
					atype = AT_LL; llv = v_num_ll(v_num(arg));
					snprintf(subfmt, sizeof subfmt, "%%%s%s%sll%c", flags, wbuf, pbuf, conv);
					break;
				case 'o': case 'u': case 'x': case 'X':
					atype = AT_ULL; ullv = (unsigned long long)v_num_ll(v_num(arg));
					snprintf(subfmt, sizeof subfmt, "%%%s%s%sll%c", flags, wbuf, pbuf, conv);
					break;
				case 'e': case 'E': case 'f': case 'F': case 'g': case 'G':
					...
					break;
				case 'c':
					if (v_numeric_ctx(arg)) cbuf[0] = (char)v_num_ll(v_num(arg));
...
static int do_sub(regex_t *re, const char *text, const char *repl, int global, char **outp)
{
	char *out = NULL;
	size_t len = 0, cap = 0;
...
static struct awk_value eval_lvalue_read(struct awk_interp *ip, struct awk_node *lv)
{
	...
	case N_VAR: {
		...
		if (c->is_array) fatal(ip, "can't read an array as a scalar");
		return cell_to_value(ip, c);
	}
	case N_FIELD: {
		struct awk_value idxv = eval(ip, lv->a);
		long idx = (long)v_num_ll(v_num(&idxv));
		v_free(&idxv);
...
static void assign_lvalue(struct awk_interp *ip, struct awk_node *lv, struct awk_value *val)
{
	...
	case N_VAR: {
		...
		if (!strcmp(lv->str, "NF")) set_nf(ip, (long)v_num_ll(cell_num(c)));
		return;
	}
	case N_FIELD: {
		struct awk_value idxv = eval(ip, lv->a);
...
static double arith(struct awk_interp *ip, int op, double a, double b)
{
	switch (op) {
	...
	case T_SLASH:
		if (b == 0.0) fatal(ip, "division by zero");
		return a / b;
	case T_PERCENT:
		if (b == 0.0) fatal(ip, "division by zero in %");
		return fmod(a, b);
...
static struct awk_value eval(struct awk_interp *ip, struct awk_node *n)
{
	...
	case N_STR: v_str_init(&v, xstrdup(n->str), VK_STR); return v;
	case N_REGEX: {
	...
	case N_BINOP: {
		struct awk_value a = eval(ip, n->a), b = eval(ip, n->b);
		double r = arith(ip, n->op, v_num(&a), v_num(&b));
		v_free(&a); v_free(&b);
		v_num_init(&v, r);
		return v;
	}
...
static struct awk_value call_builtin(struct awk_interp *ip, struct awk_node *call)
{
	...
	if (!strcmp(name, "length")) {
		if (na == 0) { v_num_init(&v, (double)strlen(ip->rec ? ip->rec : "")); return v; }
		if (a[0]->type == N_VAR) {
		...
		}
		{
			...
			v_num_init(&v, (double)strlen(v_str(&s, convfmt_str(ip))));
			v_free(&s);
			return v;
		}
	}
	...
	if (!strcmp(name, "split")) {
		...
		if (na > 2) {
			if (a[2]->type == N_REGEX) { dynre = a[2]->re; }
				...
				dynfs = xstrdup(v_str(&fsv, convfmt_str(ip)));
				v_free(&fsv);
				fs = dynfs;
			}
		} else {
	...
	if (!strcmp(name, "sub") || !strcmp(name, "gsub")) {
		...
		char *result;
		int count;
		if (target) cur = eval_lvalue_read(ip, target);
		else { v_str_init(&cur, xstrdup(ip->rec ? ip->rec : ""), VK_STR); }
		count = do_sub(re, v_str(&cur, convfmt_str(ip)), repl, !strcmp(name, "gsub"), &result);
		v_free(&cur);
		v_free(&replv);
		if (count) {
			struct awk_value nv;
...
static struct awk_value call_user_func(struct awk_interp *ip, struct awk_func *fn, struct awk_node **argnodes, int nargs)
{
	...
	if (++ip->depth > 1000) fatal(ip, "function call nesting too deep");
...
static enum awk_sig exec_stmt(struct awk_interp *ip, struct awk_node *n, struct awk_value *retval)
{
	...
	case N_EXIT: {
		if (n->a) { struct awk_value v = eval(ip, n->a); ip->exit_status = (int)v_num_ll(v_num(&v)); v_free(&v); }
		ip->exiting = 1;
		ip->unwind = SIG_EXIT;
		return SIG_EXIT;
	}
...
void awk_interp_set_str(struct awk_interp *ip, const char *name, const char *val)
{
	...
	assign_value_to_cell(c, &v);
	v_free(&v);
	if (!strcmp(name, "NF")) set_nf(ip, (long)v_num_ll(cell_num(c)));
}
