...
struct awk_interp {
	...
	/* Where a FATAL runtime condition (division by zero, an
	 * undefined-function call, a scalar used as an array or vice
	 * versa, runaway user-function recursion -- awk_run.c's fatal())
	 * unwinds to.  This is deliberately NOT the same mechanism as
	 * `unwind` above: next/exit's signal is checked cooperatively by
	 * every caller up the eval()/exec_stmt() chain, which works
	 * because every one of those callers already sits on the C stack
	 * at exactly the point that needs to see it.  A fatal condition
	 * has no such cooperative chain -- it can be discovered arbitrarily
	 * deep inside eval()'s expression recursion, or even before
	 * awk_interp_run() is ever called (awk_interp_setup_argv()'s own
	 * promote_to_array(ARGV) can hit it, if e.g. `-v ARGV=1` already
	 * bound ARGV to a scalar) -- so it uses setjmp()/longjmp() instead,
	 * set up once by __util_awk_main() (src/util/awk.c) around the
	 * whole init/setup/run sequence.  The one thing this must NOT do
	 * is call exit()/_exit(): __util_awk_main() can run in-process as
	 * a shell builtin (src/sh/builtin.c's bi_awk(), no fork), and
	 * exiting from here would tear down the whole host shell process
	 * the moment a user's one-liner divided by zero, not just this one
	 * awk invocation -- the same class of mistake src/util/dd.c's
	 * header warns about for a signal handler, applied here to an
	 * ordinary interpreter error path instead. */
	jmp_buf fatal_jmp;
