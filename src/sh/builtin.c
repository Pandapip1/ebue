...
/* ==== Tier 4: find(1p), xargs(1p), expr(1p), ls(1p) =====================
 *
 * Same reasoning as every tier above: each also exists as a real
 * standalone obj/bin/<name>.exe (src/util/<name>.c, declared in
 * src/internal/util.h), and stays registered here too so a script run
 * before PATH lookup or __spawn() can be trusted still has them --
 * which matters more than usual for `find -exec`/`xargs` specifically,
 * since both of those *themselves* depend on __find_program()/
 * __spawn() to run whatever they were told to invoke; running find/
 * xargs/expr/ls in-process here needs none of that to already work.
 * None of these four is a 2.14 special built-in and none has any effect
 * on the shell execution environment itself (2.12's list -- none of
 * them `cd`s or assigns a shell variable), so `env_effect` is 0 for all
 * four, same as every other regular built-in in this table. */
