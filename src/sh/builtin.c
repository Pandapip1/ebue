...
/* ==== Tier 4: grep(1p) ===================================================
 *
 * Same reasoning as every tier above: grep also exists as a real
 * standalone obj/bin/grep.exe (src/util/grep.c, declared in
 * src/internal/util.h), and stays registered here too so a script run
 * before PATH lookup or __spawn() can be trusted still has it.  Not a
 * 2.14 special built-in and has no effect on the shell execution
 * environment itself, so env_effect is 0, same as the rest of this
 * table. */
