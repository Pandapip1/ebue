 ...
 *
 * diff(1p), cmp(1p): same reasoning as every other block above -- each
 * also exists as a real standalone obj/bin/<name>.exe (src/util/<name>.c,
 ...
 * them.  Neither is a 2.14 special built-in and neither has any effect
 * on the shell execution environment itself (2.12's list), so
 * `env_effect` is 0 for both, same as the rest of this table. */
