 ...
 *   - without GLOB_NOSORT the vector is in strcmp order, which is what
 *     glob.html requires ("sorted according to the collating sequence",
 *     and the C locale's is byte order);
...
static void check(const char *pat, int flags, int rc, glob_t *g)
{
...
int LLVMFuzzerTestOneInput(const unsigned char *data, size_t size)
{
	...
	check(pat, flags & ~GLOB_APPEND, rc, &g);
	...
	if (rc == 0) {
		...
		check(pat, flags | GLOB_APPEND, rc2, &g);
	}
